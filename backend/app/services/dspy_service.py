...
# DSPy is an AI programming framework that helps create, compose and improve LLM prompts
# It provides tools for few-shot learning and prompt optimization
...
def get_available_llm():
    ...
    # Check for Azure OpenAI API key (using GitHub token)
...
# Initialize the LLM and configure DSPy with rate limiting
...
class ExtractStockInfo(dspy.Module):
    """
    DSPy module for extracting stock information from a text query.
    """
    def __init__(self):
        super().__init__()
...
class GenerateAnalysis(dspy.Module):
    """
    DSPy module for generating comprehensive stock analysis.
    Uses ChainOfThought for more detailed reasoning about stock metrics.
    """
    def __init__(self):
        super().__init__()
    ...
    def forward(self, stats: Dict[str, Any]) -> StockAnalysis:
        """
        Generate comprehensive stock analysis from statistical data.
        Formats and processes both fundamental and technical indicators.
        """
        # Format fundamental metrics with proper formatting and handle None values
        fundamental_factors = [
...
class DspyService:
    """
    Main service class that coordinates DSPy modules for stock analysis.
    Uses teleprompters for few-shot learning and example-based training.
    """
    def __init__(self):
        # Initialize DSPy modules for information extraction and analysis
        self.extractor = ExtractStockInfo()
        self.analyzer = GenerateAnalysis()
        
        # Configure teleprompters with bootstrap few-shot learning
        # These help improve module performance through example-based learning
        self.extract_teleprompter = dspy.teleprompt.BootstrapFewShot(
            ...
            max_rounds=2  # Number of training rounds
        )
